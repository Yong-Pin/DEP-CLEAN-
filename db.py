...
def _database_url():
    ...
    if not url:
        raise RuntimeError(
            ...
            "Paste the Supabase Session pooler URI into Render."
        )
...
def init_pool():
    ...
    if _pool is None:
        _pool = ThreadedConnectionPool(
            ...
            sslmode="require",
            connect_timeout=10,
            application_name="dep-nesso-render",
        )
...
def ensure_schema():
    statements = [
        ...
        """
        ALTER TABLE safety_alerts
        ADD COLUMN IF NOT EXISTS low_g_duration_s DOUBLE PRECISION
        """,
        """
        ALTER TABLE safety_alerts
        ...
        """
        ALTER TABLE safety_alerts
        ADD COLUMN IF NOT EXISTS feedback_label VARCHAR(30)
        """,
        """
        ALTER TABLE safety_alerts
        ...
        ON telegram_recipients (active, id)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_telegram_pairings_expiry
        ON telegram_pairings (expires_at)
        """,
        """
        ...
        ON sensor_data (device_name, timestamp DESC)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_feature_device_time
        ON feature_windows (device_name, window_end DESC)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_alert_device_time
        ON safety_alerts (device_name, received_timestamp DESC)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_replay_incident_time
        ON incident_replay_samples (incident_id, timestamp)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_pipeline_device_time
        ON pipeline_metrics (device_name, timestamp DESC)
        """,
